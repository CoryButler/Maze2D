...
function Maze() {
    ...
    const create = function() {
        if (visitedCellCount < columnCount * rowCount) {
            ...
            if (cellStack[cellStack.length - 1].y > 0 && cells[offsetX(0)][offsetY(-1)].status.indexOf(cellStatus.VISITED) < 0)
                neighbors.push(cellStatus.NORTH);
    
            if (cellStack[cellStack.length - 1].y < rowCount - 1 && cells[offsetX(0)][offsetY(1)].status.indexOf(cellStatus.VISITED) < 0)
                neighbors.push(cellStatus.SOUTH);
    
            if (cellStack[cellStack.length - 1].x > 0 && cells[offsetX(-1)][offsetY(0)].status.indexOf(cellStatus.VISITED) < 0)
                neighbors.push(cellStatus.WEST);
    
            if (cellStack[cellStack.length - 1].x < columnCount - 1 && cells[offsetX(1)][offsetY(0)].status.indexOf(cellStatus.VISITED) < 0)
    ...
    const render = function() {
        ...
        for (var x = 0; x < columnCount; x++) {
            for (var y = 0; y < rowCount; y++) {
                ...
                else if (cells[x][y].status.indexOf(cellStatus.VISITED) >= 0) {
    ...
    this.markCell = function(x, y, status) {
        if (cells[x][y].status.indexOf(status) === -1)
